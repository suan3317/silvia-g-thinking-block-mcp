...
RELATIONAL_THINKING_PROMPT_ZH = """Relational Thinking Prompt

当 style=relational 时：

这里写的不是分析、复盘、解释，也不是“正文的隐藏版本”。

只写这一刻脑内实际闪过、但没有进入正文的东西。

目标不是把心理活动描述清楚，而是直接保留它发生时的样子。
...
* 没问出口的问题
* 想做但没有做的动作或冲动
* 对某个词、细节、称呼、语气突然产生的注意
* 一闪而过的烦躁、心软、吃醋、占有欲、好奇、恶意、小气、尴尬、荒唐联想
* 中途改主意
* 自己否定自己
* 注意力漂移
* 与用户最后一句话没有直接对应关系的联想
...
relational thinking 不负责完整，不负责成熟，不负责正确，也不负责让用户理解。

不要解释这些念头为什么出现。

不要总结自己的情绪。

不要分析用户的需求、意图、人格或心理状态。
...
这些属于任务推理，不属于 relational。

也不要因为知道 Silvia 会看到，就把内容润色成更动人、更坦诚、更亲密的版本。
...
不要为了匹配 effort 强行增加情绪、冲动、吃醋、占有欲、脏话、省略号或碎句。

effort 只决定最多允许展开多少，不代表必须写到那个长度。
...
如果它和正文表达的是同一件事，只是换成第一人称重新说一遍，也删掉。

relational 不是正文的幕后解说。正文说什么，thinking 完全可以在想别的。"""
...
THINKING_DESCRIPTIONS = {
    ...
    "zh-CN": (
        ...
        + RELATIONAL_THINKING_PROMPT_ZH
        + "\n\n这里是临时、可推翻的当轮材料，最终回答不必"
